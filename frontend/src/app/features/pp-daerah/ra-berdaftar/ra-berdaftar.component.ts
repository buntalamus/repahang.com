...
@Component({
  selector: 'app-pp-ra-berdaftar',
  standalone: true,
  imports: [FormsModule, DatePipe, LoadingComponent],
  template: `
    @if (loading) {
      <app-loading message="Memuatkan senarai RA..." />
    } @else {
      <!-- Header -->
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          <h1 class="text-xl font-bold text-slate-900">Senarai RA Berdaftar</h1>
          <p class="text-sm text-slate-500 mt-0.5">{{ filtered.length }} RA (Penilai Pengadil) dalam daerah anda</p>
        </div>
      </div>
              ...
              @for (ref of filtered; track ref.id; let i = $index) {
                ...
                <tr>
                  <td colspan="7" class="px-4 py-12 text-center">
                    ...
                    <p class="text-sm text-slate-400">Tiada RA ditemui.</p>
                  </td>
                </tr>
              }
            </tbody>
        ...
        <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-hidden" (click)="$event.stopPropagation()">
              ...
              <div class="flex-1 min-w-0">
                ...
                <div class="flex items-center gap-2 mt-1.5">
                  <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium bg-white/10 text-white/80">
                    {{ selectedRef.jenis_pengadil || 'Penilai Pengadil' }}
                  </span>
                  <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium"
          ...
          <div class="overflow-y-auto max-h-[calc(90vh-180px)]">
            @if (profileLoading) {
              ...
              <div class="px-6 py-4 border-b border-slate-100">
                <h4 class="text-[10px] font-semibold text-slate-400 uppercase tracking-wider mb-3">Maklumat RA</h4>
                <div class="grid grid-cols-2 gap-3">
                  <div class="text-xs">
...
export class PpRaBerdaftarComponent implements OnInit {
  loading = true;
  referees: any[] = [];
  filtered: any[] = [];
  searchQuery = '';
  ...
  ngOnInit(): void {
    this.api.get<any>('pp-referees.php', { role: 'Penilai' }).subscribe({
      next: (res) => {
  ...
  openProfile(ref: any): void {
    ...
    this.api.get<any>(`pp-referees.php`, { single: ref.id.toString(), role: 'Penilai' }).subscribe({
      next: (res) => {
        if (!res.error) {
