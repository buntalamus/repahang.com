<?php
/**
 * R3-A - BORANG PERMOHONAN PENILAI PENGADIL BARU / DATA DIRI PENILAI PENGADIL
 * Borang Ujian Kelas 1 FAM
 * Format mengikut Borang R3(A) rasmi FAM
 */
function generateR3APdf($pdf, $data) {
    $pdf->setCreator(PDF_CREATOR);
    $pdf->setAuthor('Persatuan Bolasepak Negeri Pahang');
    $pdf->setTitle('Borang R3-A - Data Diri Penilai Pengadil - ' . strtoupper($data['nama_penuh'] ?? ''));
    $pdf->setSubject('Borang R3-A');
    ...
    // --- R3(A) label top right ---
    $pdf->setFont('helvetica', 'B', 10);
    $pdf->Cell(0, 5, 'R3 (A)', 0, 1, 'R');
    $pdf->ln(1);

    // --- No. Rujukan & Tarikh ---
    $no_rujukan_r3a = 'PBNP/R3A/' . $tahun . '/' . str_pad($data['id'] ?? 0, 4, '0', STR_PAD_LEFT);
    $pdf->setFont('helvetica', '', 9);
    $pdf->Cell(100, 5, 'No. Rujukan: ' . $no_rujukan_r3a, 0, 0, 'L');
    $pdf->Cell(0, 5, 'Tarikh: ' . date('d/m/Y'), 0, 1, 'R');
    $pdf->ln(2);

    // --- Title ---
    $pdf->setFont('helvetica', 'B', 12);
    $pdf->Cell(0, 7, 'BORANG PERMOHONAN PENILAI PENGADIL BARU', 0, 1, 'L');
    $pdf->ln(2);
