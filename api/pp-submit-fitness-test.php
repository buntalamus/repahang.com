...
try {
    ...
    // Check if referee already has a fitness test application this year
    $tahun_permohonan = (int)$_POST['tahun_permohonan'];
    $stmt = $pdo->prepare("SELECT id FROM permohonan WHERE user_id = ? AND tahun_permohonan = ? AND jenis_permohonan = 'ujian_kecergasan'");
    $stmt->execute([$user_id, $tahun_permohonan]);
    if ($stmt->fetch()) {
        throw new Exception('Referee already has a fitness test application for this year');
    }
    ...
    $data = [
        ...
        'jenis_borang' => 'ujian_kecergasan',
        'jenis_permohonan' => 'ujian_kecergasan',
    ...
    $declaration_fields = ['declare1', 'declare2', 'declare3', 'declare4', 'declare5'];
    ...
    echo json_encode([
        'error' => false,
        'message' => 'Fitness test application submitted successfully',
