...
try {
    ...
    if (!$user_id || !$tahun_permohonan) {
        throw new Exception('Medan wajib tidak lengkap');
    }

    // Check if application already exists for this user and year
    $stmt = $pdo->prepare("SELECT id FROM permohonan WHERE user_id = ? AND tahun_permohonan = ? AND jenis_borang = 'pengadil_futsal'");
    $stmt->execute([$user_id, $tahun_permohonan]);
    if ($stmt->fetch()) {
        throw new Exception('Permohonan pengadil futsal sudah wujud untuk tahun ini');
    }
    ...
    jsonResponse([
        'error' => false,
        'message' => 'Permohonan pengadil futsal berjaya dihantar',
